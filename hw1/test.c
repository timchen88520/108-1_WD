...
void handle_socket(int fd) {
  ...
  /* 只接受 GET 命令要求 */
  if (strncmp(buffer, "GET ", 4) && strncmp(buffer, "get ", 4)) exit(3);
  ...
  if ((file_fd = open(&buffer[5], O_RDONLY)) == -1)
    write(fd, "Failed to open file that ", 19);

  /* 傳回瀏覽器成功碼 200 和內容的格式 */
  sprintf(buffer, "here  HTTP/1.0 200 OK\r\nContent-Type: %s\r\n\r\n", fstr);
...
int sendphoto(int *pFD){
  int fd=*pFD;
  FILE *fp;
  int len;

  char readbuff[1024*578]={0};//set all as 0
  ...
  tlen=sprintf(p_bufs,"HTTP/1.1 200 OK\r\n"
                      ...
                      "Connection: close\r\n\r\n",
                      len);
  memcpy(p_bufs+tlen,readbuff,len);
  tlen+=len;
...
main(int argc, char **argv) {
  ...
  /* 開始監聽網路 */
  if (listen(listenfd, 64) < 0) exit(3);
  //FILE *photo;
  //photo=fopen("shi.jpg","r");
  while (1) {
